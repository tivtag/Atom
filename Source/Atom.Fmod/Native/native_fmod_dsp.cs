...
namespace Atom.Fmod.Native
{
    ...
    /* 
        DSP callbacks
    */
    internal delegate RESULT DSP_CREATECALLBACK( ref DSP_STATE dsp_state );
    internal delegate RESULT DSP_RELEASECALLBACK( ref DSP_STATE dsp_state );
    internal delegate RESULT DSP_RESETCALLBACK( ref DSP_STATE dsp_state );
    internal delegate RESULT DSP_READCALLBACK( ref DSP_STATE dsp_state, IntPtr inbuffer, IntPtr outbuffer, uint length, int inchannels, int outchannels );
    internal delegate RESULT DSP_SETPOSITIONCALLBACK( ref DSP_STATE dsp_state, uint seeklen );
    internal delegate RESULT DSP_SETPARAMCALLBACK( ref DSP_STATE dsp_state, int index, float val );
    internal delegate RESULT DSP_GETPARAMCALLBACK( ref DSP_STATE dsp_state, int index, ref float val, StringBuilder valuestr );
    internal delegate RESULT DSP_DIALOGCALLBACK( ref DSP_STATE dsp_state, IntPtr hwnd, bool show );
    ...
    */
    ////internal struct DSP_PARAMETERDESC
    ////{
    ////    internal float min;             /* [in] Minimum value of the parameter (ie 100.0). */
    ////    internal float max;             /* [in] Maximum value of the parameter (ie 22050.0). */
    ////    internal float defaultval;      /* [in] Default value of parameter. */
    ////    [MarshalAs( UnmanagedType.ByValArray, SizeConst = 16 )]
    ////    internal char[] name;            /* [in] Name of the parameter to be displayed (ie "Cutoff frequency"). */
    ////    [MarshalAs( UnmanagedType.ByValArray, SizeConst = 16 )]
    ////    internal char[] label;           /* [in] Short string to be put next to value to denote the unit type (ie "hz"). */
    ////    internal string description;     /* [in] Description of the parameter to be displayed as a help item / tooltip for this parameter. */
    ////}
    ...
    */
    internal struct DSP_DESCRIPTION
    {
        ...
        internal char[] name;               /* [in] Name of the unit to be displayed in the network. */
        internal uint version;            /* [in] Plugin writer's version number. */
        internal int channels;           /* [in] Number of channels.  Use 0 to process whatever number of channels is currently in the network.  >0 would be mostly used if the unit is a unit that only generates sound. */
        internal DSP_CREATECALLBACK create;             /* [in] Create callback.  This is called when DSP unit is created.  Can be null. */
        internal DSP_RELEASECALLBACK release;            /* [in] Release callback.  This is called just before the unit is freed so the user can do any cleanup needed for the unit.  Can be null. */
        internal DSP_RESETCALLBACK reset;              /* [in] Reset callback.  This is called by the user to reset any history buffers that may need resetting for a filter, when it is to be used or re-used for the first time to its initial clean state.  Use to avoid clicks or artifacts. */
        internal DSP_READCALLBACK read;               /* [in] Read callback.  Processing is done here.  Can be null. */
        internal DSP_SETPOSITIONCALLBACK setposition;        /* [in] Setposition callback.  This is called if the unit wants to update its position info but not process data.  Can be null. */

        internal int numparameters;      /* [in] Number of parameters used in this filter.  The user finds this with DSP::getNumParameters */
        internal IntPtr paramdesc;          /* [in] Variable number of parameter structures. */
        internal DSP_SETPARAMCALLBACK setparameter;       /* [in] This is called when the user calls DSP::setParameter.  Can be null. */
        internal DSP_GETPARAMCALLBACK getparameter;       /* [in] This is called when the user calls DSP::getParameter.  Can be null. */
        internal DSP_DIALOGCALLBACK config;             /* [in] This is called when the user calls DSP::showConfigDialog.  Can be used to display a dialog to configure the filter.  Can be null. */
        internal int configwidth;        /* [in] Width of config dialog graphic if there is one.  0 otherwise.*/
        internal int configheight;       /* [in] Height of config dialog graphic if there is one.  0 otherwise.*/
        internal IntPtr userdata;           /* [in] Optional. Specify 0 to ignore. This is user data to be attached to the DSP unit during creation.  Access via DSP::getUserData. */
    }
    ...
    */
    internal enum DSP_LOWPASS_SIMPLE
    {
